...
import { getRandomIntFromRange } from './src/utils.js'
import { monsterNames, messages, buttonMsg } from './src/data.js'
import { makeHeart } from './src/heart.js';
...
const heartOne = makeHeart("#heart1");
const heartTwo = makeHeart("#heart2");
const heartThree = makeHeart("#heart3");
let lostCounter = 0;
...
const generateMonster = (playerStats) => {
    if (playerStats.level == 1) {
        ...
        (monsterLevelStats = getRandomIntFromRange(1, 4)),
        (monsterStats = {
            //Example monster's stats stored in object
            power: getRandomIntFromRange(1, monsterLevelStats + 4),
            toughness: getRandomIntFromRange(1, monsterLevelStats + 4),
    ...
    if (playerStats.level == 2) {
        selectedMonster =
            monsterNames[Math.floor(Math.random() * monsterNames.length)];
        (monsterLevelStats = getRandomIntFromRange(1, 5)),
        (monsterStats = {
            //Example monster's stats stored in object
            power: getRandomIntFromRange(1, monsterLevelStats + 4),
            toughness: getRandomIntFromRange(1, monsterLevelStats + 4),
    ...
    if (playerStats.level == 3) {
        selectedMonster =
            monsterNames[Math.floor(Math.random() * monsterNames.length)];
        (monsterLevelStats = getRandomIntFromRange(1, 6)),
        (monsterStats = {
            //Example monster's stats stored in object
            power: getRandomIntFromRange(1, monsterLevelStats + 4),
            toughness: getRandomIntFromRange(1, monsterLevelStats + 4),
    ...
    selectedMonster =
        monsterNames[Math.floor(Math.random() * monsterNames.length)];
    (monsterLevelStats = getRandomIntFromRange(
        playerStats.level - 2,
        playerStats.level + 3
    )),
    (monsterStats = {
        //Example monster's stats stored in object
        power: getRandomIntFromRange(monsterLevelStats - 1, monsterLevelStats + 4),
        toughness: getRandomIntFromRange(monsterLevelStats - 1, monsterLevelStats + 4),
        name: selectedMonster.name,
        pic: selectedMonster.pic,
        hp: 100,
    });
